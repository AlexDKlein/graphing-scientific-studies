def to_gexf(g, path, vertex_labels='id', edge_labels='id'):
  ...
  <graph mode="static" defaultedgetype="directed" name="">
    <nodes>\n"""
    string += ''.join(g.vertices.rdd.map(lambda v: f'      <node id="{v["id"]}" label="{v[vertex_labels]}" />\n').collect())
    string += """    </nodes>
    <edges>\n"""
    string += ''.join(g.edges.rdd.map(lambda e: f'      <edge id="{e["id"]}" source="{e["src"]}" target="{e["dst"]}" label="{e[edge_labels]}" />\n').collect())
    string += """    </edges>
  </graph>
</gexf>"""
